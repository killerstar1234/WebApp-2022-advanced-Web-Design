$('#preForm').click(() => {
    ...
    <select id="select">
        ...
        <option>Music</option>
        <option>Physical Eduction</option>
        <option>Communication Arts</option>
        <option>Science</option>
        <option>Social Studies</option>
        <option>Tech and Engineering Education</option>
      </select>
...
<div id="subjectResults" class = 'container mt-4'></div>
    `)
...
<button id = 'findHelp' onclick = 'findHelp()'>Submit</button>
`)
    }
...
function findHelp() {
    ...
    const newCat = catigory.split(' ').join('').toLowerCase()
    const name = document.getElementById('name').value;
    const email = document.getElementById('email').value;

    $('#results').empty();
    $('#results').append('<h4>Loading...</h4>')
    

    // Send The data to the server

    fetch('http://localhost:3000/add/subject', {
        method: 'POST',
        body: JSON.stringify({
            catigory: newCat,
            name: name,
